...
Higher even amount of cylinders is associated with lower mpg. An odd amount of cylinders is associated with lower mpg than an even amount of cylinders.
...
We are graphing the amount of cylinders versus mpg shown as a boxplot. 
This is a box plot shows the minimum, first quartile, median, third quartile,and maximum. The red box shows the first quantile on the very left of each red box, a bold line for the mean, and the very right is the third quantile. 
If a car has four cylinders, it gets the most range of mpg out of all the cylinders. This means that the amount of cylinders in a car changes the range of mpg that the car gets. 
  **E**xtra information (optional): anything else that you find interesting to point out
...
```
  ...
  **T**opic: what data (observational units and variables) you are graphing
  We are graphing the range of mpg a car gets.
  **A**xes: what variables are being mapped to the x-axis and y-axis (and other properties, e.g., color)
  The x-axis is the frequency of mpg of all cars, while the y-axis is the amount of mpg.
  **M**ain point: the most important takeaway (for one variable, this usually deals with the mode of the distribution; for two variables, this usually deals with the association)
  The cars are likely to have mpg from 12 to 38.
  **E**xtra information (optional): anything else that you find interesting to point out
...
```{r load ggplot2, message = FALSE, warning = FALSE}
library(ggplot2)
```
