...
```{r A2c function}
assertive_indirect_effect <- function(df){
  non_caus_fit <- lm(assertive_behavior ~ beautification, data = df)
  caus_fit <- lm(sexual_motivation ~ beautification, data = df)
  mult_fit <- lm(assertive_behavior ~ sexual_motivation + beautification, data = df)
...
```{r A2c}
assertive_boot <- boot(data = assertive_woman2, statistic = assertive_indirect_effect, R = 1000, formula = assertive_woman2 )
